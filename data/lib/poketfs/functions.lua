...
balls = {
pokeball = {emptyId = 2147, usedOn = 2520, usedOff = 2521, effectFail = 5, effectSucceed = 10, missile = 6, effectRelease = 7, chanceMultiplier = 1.0},
...
function statusGainFormula(playerLevel, summonLevel, summonBoost, pokeLove)
	--print((1.0 + summonLevel * summonLevelDamageBuff + playerLevel * playerLevelDamageBuff + summonBoost * summonBoostDamageBuff + pokeLove * summonLoveDamageBuff))
...
function damageFormulaMin(playerLevel, summonLevel, summonBoost, pokeLove)
	return statusGainFormula(playerLevel, summonLevel, summonBoost, pokeLove)
end
function damageFormulaMax(playerLevel, summonLevel, summonBoost, pokeLove)
	return (playerLevel + summonBoost)
end
...
function Monster.getTotalHealth(self)
	local monsterType = MonsterType(self:getName())
		local levelPoke = monsterType:getPokemonLevel()
		local hpGain = monsterType:getPokemonGainHp()
		local boostNivel = monsterType:getPokemonBoost()
	if self:isPokemon() then
		local playerLevel = self:getMasterLevel()
	--	print('levelPoke: ' .. levelPoke)
		
	--	print('hpStatus: ' .. hpStatus)
		
	--	print('hpGain: ' .. hpGain)
		
	--	print('boostNivel: ' .. boostNivel)
		
	--	print('level: ' .. playerLevel)
	--HP = (IVHP + 10) * 2 * N�vel do Monstro *                                       (1 + (N�vel do Monstro / 100)) + Quantidade de HP por N�vel
		local total = math.floor( monsterType:getMaxHealth() + ((playerLevel+boostNivel) * hpGain) + (hpGain * 0.70 ))
		-- * (1 + ((boostNivel + playerLevel)/100))) + hpGain
	...
	elseif self:isMonster() then
			local total = math.floor( monsterType:getMaxHealth())
			print('\ntotal' .. total)
		return total
	end
	return 0
end
function MonsterType.getTotalHealth(self)
	local monsterType = MonsterType(self:getName())
		local levelPoke = monsterType:getPokemonLevel()
		local hpGain = monsterType:getPokemonGainHp()
		local boostNivel = monsterType:getPokemonBoost()
...
function Monster.getTotalHealthVocationContribution(self)
	...
	if self:isPokemon() then
		local playerLevel = self:getMasterLevel()
		if self:getMaster():getVocation():getName() == "Blocker" then
			return math.floor(monsterType:getMaxHealth() * statusGainFormula(playerLevel, self:getLevel(), self:getPokemonBoost(), self:getLove()) * (blockerHealthBuff - 1.0))
		end
	end
	return 0
end
...
function Monster.getTotalHealthPlayerContribution(self)
	...
	if self:isPokemon() then 
		local playerLevel = self:getMasterLevel()
		return math.floor(monsterType:getMaxHealth() * (statusGainFormula(playerLevel, 0, 0, 0) - statusGainFormula(0, 0, 0, 0)))
	end
	return 0
end
...
function Monster.getTotalHealthSummonLevelContribution(self)
	...
	if self:isPokemon() then 
		local playerLevel = self:getMasterLevel()
		return math.floor(monsterType:getMaxHealth() * (statusGainFormula(0, playerLevel, 0, 0) - statusGainFormula(0, 0, 0, 0)))
	elseif self:isMonster() then
		return math.floor(monsterType:getMaxHealth() * (statusGainFormula(0, playerLevel, 0, 0) - statusGainFormula(0, 0, 0, 0)))	
	end	
	return 0
...
function Monster.getTotalHealthSummonBoostContribution(self)
	...
	if self:isPokemon() then 
		...
		return math.floor(monsterType:getMaxHealth() * (statusGainFormula(0, 0, monsterType:getPokemonBoost(), 0) - statusGainFormula(0, 0, 0, 0)))
	end
	return 0
end
...
function Monster.getTotalMaxMeleeAttackPlayerContribution(self)
	local monsterType = MonsterType(self:getName())
	local minMelee = 0
	local maxMelee = 0
	...
	if self:isPokemon() then 
		local playerLevel = self:getMasterLevel()
		return math.floor(aveMelee * (statusGainFormula(playerLevel, 0, 0, 0) - statusGainFormula(0, 0, 0, 0)))
	end
	return 0
end
...
function Monster.getTotalMeleeAttackSummonLevelContribution(self)
	...
	local aveMelee = (minMelee + maxMelee) / 2
	if self:isPokemon() then 
		return math.floor(aveMelee * (statusGainFormula(0, monsterType:getPokemonLevel(), 0, 0) - statusGainFormula(0, 0, 0, 0)))
	elseif self:isMonster() then
		return math.floor(aveMelee * (statusGainFormula(0, monsterType:getPokemonLevel(), 0, 0) - statusGainFormula(0, 0, 0, 0)))
	end
	return 0
...
function Monster.getTotalMeleeAttackSummonBoostContribution(self)
	...
	local aveMelee = (minMelee + maxMelee) / 2
	if self:isPokemon() then 
		return math.floor(aveMelee * (statusGainFormula(0, 0, monsterType:getPokemonBoost(), 0) - statusGainFormula(0, 0, 0, 0)))
	end
	return 0
end
...
function Monster.getTotalMeleeAttackVocationContribution(self)
	...
	if self:isPokemon() then 
		...
		if self:getMaster():getVocation():getName() == "Hunter" then
			return math.floor(aveMelee * statusGainFormula(playerLevel, monsterType:getPokemonLevel(), monsterType:getPokemonBoost(), self:getLove()) * (hunterDamageBuff - 1.0))
		end
	end
	return 0
end
...
function getTotalAttack(self)
	print('getTotalAttack' .. self:getName())
	local monsterType = MonsterType(self:getName())
	if self:isPokemon() then
		local total = math.floor( (monsterType:getAttack() * 2) + (monsterType:getPokemonLevel() + monsterType:getPokemonBoost() + self:getLove()) / (monsterType:getPokemonDPS()))
		print('getTotalAttack : ' .. total .. ' name: ' .. self:getName())
		return total
	elseif self:isMonster() then
		local total = math.floor( (monsterType:getAttack() * 2) + (monsterType:getPokemonLevel() + 30) / (monsterType:getPokemonDPS()))
		print('getTotalAttack : ' .. total .. ' name: ' .. self:getName())
		return total
	end
	return 0
end
function getTotalHealthPokemon(player, itemTarget)
	local monsterType = MonsterType(self:getName())
	print('monsterType: ' .. monsterType:getMonster())
	if monsterType:isPokemon() then
		local levelPoke = monsterType:getPokemonLevel()
		local hpStatus = monsterType:getPokemonHealthStatus()
		local hpGain = monsterType:getPokemonGainHp()
		local boostNivel = monsterType:getPokemonBoost()
		local playerLevel = monsterType:getMasterLevel()
		print('levelPoke: ' .. levelPoke)
		...
		print('level: ' .. playerLevel)
		local total = math.floor((monsterType:getHealth() * 2 + hpStatus + 50 + (boostNivel + playerLevel) * hpGain))
		if monsterType:getMaster():getVocation():getName() == "Blocker" then
			total = total * blockerHealthBuff
		end
		local vitamins = monsterType:getUsedVitaminsNumber("hp")
		if vitamins > 0 then
			total = total + math.floor(monsterType:getMaxHealth() * vitamins / maxVitamins * vitaminStatusBuff)
		end
		print('\ntotal' .. total)
		return total
	elseif monsterType:isMonster() then
		return math.floor(monsterType:getMaxHealth() * statusGainFormula(0, monsterType:getPokemonLevel(), 0, 0))
	end
	return 0
end
...
print('getTotalMagicAttackVocationContribution' .. self:getName())
	return 0
end

function Monster.getTotalMagicAttackPlayerContribution(self)
print('getTotalMagicAttackPlayerContribution' .. self:getName())
	return 0
end

function Monster.getTotalMagicAttackSummonLevelContribution(self)
print('getTotalMagicAttackSummonLevelContribution' .. self:getName())
	return 0
end

function Monster.getTotalMagicAttackSummonBoostContribution(self)
print('getTotalMagicAttackSummonBoostContribution' .. self:getName())
return 0
end

function Monster.getTotalMagicAttackVitaminContribution(self)
print('getTotalMagicAttackVitaminContribution' .. self:getName())
	return 0
end
...
function Monster.getTotalDefense(self)
	...
	if self:isPokemon() then
		local playerLevel = self:getMasterLevel()
		--defesa = total_defesa / (total_defesa + (level * 5)) * (level * 2)
		local total =  math.floor(monsterType:getDefense()+ 5)
...
function Monster.getTotalDefensePlayerContribution(self)
	...
	if self:isPokemon() then 
		local playerLevel = self:getMasterLevel()
		return math.floor(monsterType:getDefense() * (statusGainFormula(playerLevel, 0, 0, 0) - statusGainFormula(0, 0, 0, 0)))
	end
	return 0
end
...
function Monster.getTotalDefenseSummonLevelContribution(self)
	local monsterType = MonsterType(self:getName())
	if self:isPokemon() then 
		return math.floor(monsterType:getDefense() * (statusGainFormula(0, monsterType:getPokemonLevel(), 0, 0) - statusGainFormula(0, 0, 0, 0)))
	elseif self:isMonster() then
		return math.floor(monsterType:getDefense() * (statusGainFormula(0, monsterType:getPokemonLevel(), 0, 0) - statusGainFormula(0, 0, 0, 0)))
	end
	return 0
...
function Monster.getTotalDefenseSummonBoostContribution(self)
	local monsterType = MonsterType(self:getName())
	if self:isPokemon() then 
		return math.floor(monsterType:getDefense() * (statusGainFormula(0, 0, monsterType:getPokemonBoost(), 0) - statusGainFormula(0, 0, 0, 0)))
	end
	return 0
end
...
function Monster.getTotalSpeed(self)
	local monsterType = MonsterType(self:getName())
	if self:isPokemon() then 
		print('self name: ' .. monsterType:getName())
		local playerLevel = self:getMasterLevel()
		print('playerLevel: ' .. playerLevel)
		
		local total = math.floor(monsterType:getBaseSpeed() * statusGainFormula(playerLevel, monsterType:getPokemonLevel(), monsterType:getPokemonBoost(), self:getLove()))
		...
		return total
	elseif self:isMonster() then
		return math.floor(monsterType:getBaseSpeed() * statusGainFormula(0, monsterType:getPokemonLevel(), 0, 0))
	end
	return 0
end
...
function Monster.getTotalSpeedPlayerContribution(self)
	...
	if self:isPokemon() then 
		local playerLevel = self:getMasterLevel()
		return math.floor(monsterType:getBaseSpeed() * (statusGainFormula(playerLevel, 0, 0, 0) - statusGainFormula(0, 0, 0, 0)))
	end
	return 0
end
...
function Monster.getTotalSpeedSummonLevelContribution(self)
	local monsterType = MonsterType(self:getName())
	if self:isPokemon() then 
		return math.floor(monsterType:getBaseSpeed() * (statusGainFormula(0, monsterType:getPokemonLevel(), 0, 0) - statusGainFormula(0, 0, 0, 0)))
	elseif self:isMonster() then
		return math.floor(monsterType:getBaseSpeed() * (statusGainFormula(0, monsterType:getPokemonLevel(), 0, 0) - statusGainFormula(0, 0, 0, 0)))
	end
	return 0
...
function Monster.getTotalSpeedSummonBoostContribution(self)
	local monsterType = MonsterType(self:getName())
	if self:isPokemon() then 
		return math.floor(monsterType:getBaseSpeed() * (statusGainFormula(0, 0, monsterType:getPokemonBoost(), 0) - statusGainFormula(0, 0, 0, 0)))
	end
	return 0
end
...
function Player.resetMoveExhaustion(self, ballKey)
--	local item = self:getSlotItem(CONST_SLOT_AMMO)
	local item = ballKey
	if item then
		local summonName = item:getSpecialAttribute("pokeName")
		local monsterType = MonsterType(summonName)
		local moveList = monsterType:getMoveList()
		for i = 1, #moveList do
			local move = "cd" .. i
			item:setSpecialAttribute(move, os.time())
		end
	end
	return false
end
function Player.resetHealthPokemon(self, item)
--	local item = self:getSlotItem(CONST_SLOT_AMMO)
	local item = Item(item.uid)
	if item then
		item:setSpecialAttribute("pokeHealth", 100000)
	end
	return false
end
...
--		local item = master:getSlotItem(CONST_SLOT_AMMO)
		...
		print('pokeLevel: ' .. self:getLevel() )
		if pokeLevel ~= nil then			
			return pokeLevel
		end
	elseif isMonster(self) then
		return self:getPokemonLevel()
	end
	return 1
end
function Monster.getSummonLevel(self)
...
function doEvolveSummon(player, poke, evolutionName, ancient)
	if not ancient then ancient = false end
	local monsterType = MonsterType(evolutionName)
	local summon = Creature(poke)
	local summonPos = summon:getPosition()
--	local item = master:getSlotItem(CONST_SLOT_AMMO)
	local item = player:getUsingBall()
	if monsterType and summon:isMonster() then
	local pokeLevel = MonsterType(evolutionName):getPokemonLevel()
	player:say("Wow, my " .. summon:getName() .. " is evolving!", TALKTYPE_SAY)
	player:getPosition():sendMagicEffect(292)
	player:setStorageValue(storageEvolving, 1)
	summon:changeSpeed(-summon:getSpeed())
	item:setSpecialAttribute("pokeName", evolutionName)
	item:setSpecialAttribute("pokeLevel", pokeLevel)
	item:setSpecialAttribute("pokeBoost", 0)
		if ancient then
			item:setSpecialAttribute("owner", player:getName())
			player:setStorageValue(storageEvolutionAncient, 1)
		end
...
--		end
		...
		local newMaxHealth = math.floor(baseHealth * statusGainFormula(player:getLevel(), pokeLevel, item:getSpecialAttribute("pokeBoost"), item:getSpecialAttribute("pokeLove") or 0))
		...
		addEvent(doRemoveSummon, 4000, player:getId(), false, nil, false)
		addEvent(doReleaseSummon, 4001, player:getId(), summonPos, false, false)
		player:refreshPokemonBar({}, {})
		return true
	else
		print("WARNING! Problem on evolution to " .. evolutionName .. " player " .. player:getName())
	end
	return false
end
...
function Item.isUsedPokeball(self)
	for key, value in pairs(balls) do
		if value.usedOn == self:getId() then
			return true			
		end
	end
	return false
...
--				print("WARNING! Player " .. player:getName() .. " sending pokemon " .. name .. " to CP after first try.")
		...
		if addBall then
			...
			print('baseHealth add ball '.. baseHealth)
			local maxHealth = math.floor(baseHealth * statusGainFormula(player:getLevel(), monsterType:getPokemonLevel(), boost, 0))
...
function Player:canReleaseSummon(pokeLevel, pokeBoost, ownerName)
	...
	--print('boost: ' .. pokeBoost)
	if playerLevel < pokeLevel+pokeBoost then
		self:sendCancelMessage("Sorry, not possible. You need level " .. pokeLevel+pokeBoost .. " to use this pokemon.")
		return false
	end
