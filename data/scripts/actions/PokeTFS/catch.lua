...
function catch.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local chance = catchChances[name]
	if chance == nil then
		print("DEBUG: chance is nil for monster " .. name)
		playerPos:sendMagicEffect(CONST_ME_POFF)
		player:sendCancelMessage("Sorry, it is impossible to catch this monster.")
		return false
	else
		chance = chance * balls[ballKey].chanceMultiplier
		print("DEBUG: chance for monster " .. name .. " is " .. chance)
		return true
	end
	...
	if math.random(1, 300) <= chance then -- caught
		print("DEBUG: monster " .. name .. " was caught")
		...
		if player:getSlotItem(CONST_SLOT_BACKPACK) and player:getSlotItem(CONST_SLOT_BACKPACK):getEmptySlots() >= 1 and player:getFreeCapacity() >= 1 then -- add to backpack
			...
			print("DEBUG: monster " .. name .. " was missed")
